...
DataPage::DataPage(QWidget *parent) : QWidget(parent)
{
    ...
    table->setHorizontalHeaderLabels({"Plate", "Well", "# Sites", "# Images", "# Images Quant.", "# Cells"});

    layout->addWidget(refresh_button);
    layout->addWidget(table);

    QObject::connect(refresh_button, &QPushButton::pressed, this, &DataPage::refreshContent);
}

void DataPage::setExperimentControlModel(ExperimentControlModel *model)
{
    this->model = model;
    QObject::connect(model, &ExperimentControlModel::quantificationUpdated, this, &DataPage::refreshContent);
}
...
void DataPage::refreshContent()
{
    ...
    for (const auto & plate : exp->Samples()->Plates()) {
        ...
        for (const auto & well : plate->EnabledWells()) {
            std::vector<NDImage *> ims = exp->Images()->ListNDImage(plate->ID(), well->ID());
    ...
    QStringList header_labels = {"Plate", "Well", "# Sites", "# Images", "# Images Quant.", "# Cells", "# Cells/Image"};
        ...
        for (int i_well = 0; i_well < enabled_wells.size(); i_well++, i_row++) {
            ...
            for (const auto & im : ims) {
                if (exp->Analysis()->HasQuantification(im->Name(), 0)) {
                    n_quantified++;
                    QuantificationResults quantification = exp->Analysis()->GetQuantification(im->Name(), 0);
                    ...
                    if (raw_intensity_mean.empty()) {
                        ...
                        for (int i_ch = 0; i_ch < ch_names.size(); i_ch++) {
                            raw_intensity_mean[i_ch] = xt::hstack(
                                xt::xtuple(raw_intensity_mean[i_ch],
            ...
            if (n_quantified > 0) {
                set_col_fn(6, fmt::format("{:.1f}", (float)n_cells/(float)n_quantified));
            }
            
            if ((!raw_intensity_mean.empty()) && (ch_names.size() == raw_intensity_mean.size())) {
                for (int i_ch = 0; i_ch < ch_names.size(); i_ch++) {
                    double ch_mean = xt::pow(10, xt::mean(xt::log10(raw_intensity_mean[i_ch])))[0];
