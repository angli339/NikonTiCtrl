...
static tmsize_t _tiffosReadProc(thandle_t fd, void *buf, tmsize_t size);
...
static tmsize_t _tiffosReadProc(thandle_t fd, void *buf, tmsize_t size)
{
    tiffios_data *data = reinterpret_cast<tiffios_data *>(fd);
...
static int _tiffosCloseProc(thandle_t fd)
{
    ...
    delete reinterpret_cast<tiffios_data *>(fd);
    return 0;
}
...
static TIFF *_tiffStreamOpen(const char *name, const char *mode, void *fd)
{
    ...
    if (strchr(mode, 'w')) {
        ...
        data->stream = reinterpret_cast<iostream *>(fd);
        data->start_pos = data->stream->tellp();

        // Open for writing.
        tif = TIFFClientOpen(name, mode, reinterpret_cast<thandle_t>(data),
                             _tiffosReadProc, _tiffosWriteProc, _tiffosSeekProc,
                             _tiffosCloseProc, _tiffosSizeProc,
        ...
        tif = TIFFClientOpen(name, mode, reinterpret_cast<thandle_t>(data),
                             ...
                             _tiffisCloseProc, _tiffisSizeProc,
                             _tiffDummyMapProc, _tiffDummyUnmapProc);
        if (!tif) {
            delete data;
        }
...
TIFF *TIFFStreamOpenWrite(const char *name, iostream *os)
{
...
TIFF *TIFFStreamOpenRead(const char *name, istream *is)
{
    // NB: We don't support mapped files with streams so add 'm'
