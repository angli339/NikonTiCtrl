...
WMI::WMI()
{
    ...
    if (FAILED(hres)) {
        ...
        CoUninitialize();
        throw std::runtime_error(fmt::format("ConnectServer: Error {:#10x}", (uint32_t)hres));
    }
    ...
    if (FAILED(hres)) {
        ...
        CoUninitialize();
        throw std::runtime_error(fmt::format("CoSetProxyBlanket: Error {:#10x}", (uint32_t)hres));
    }
}
...
std::vector<std::string> WMI::listDeviceID(std::string queryWhere)
{
    ...
    std::wstring query = std::wstring(L"SELECT DeviceID FROM Win32_PnPEntity WHERE ")
...
{
    if ((vid == "") && (pid == "")) {
        return listDeviceID("(DeviceID LIKE 'USB\\\\VID_%') and (Service != 'usbhub')");
    } else if ((vid != "") && (pid == "")) {
        return listDeviceID("(DeviceID LIKE 'USB\\\\VID_" + vid + "&PID_%') and (Service != 'usbhub')");
    } else if ((vid != "") && (pid != "")) {
        return listDeviceID("(DeviceID LIKE 'USB\\\\VID_" + vid + "&PID_" + pid + "%') and (Service != 'usbhub')");
    } else {
